...
The allocator features compile the same acceptance source into three separate process-global allocator runners. Their `allocation-pressure` workload executes 24 rounds of 192 bounded live blocks with deterministic size, growth, mutation, release, and result sampling. CI requires one shared correctness digest plus three distinct executable and dependency fingerprints. Its gauges describe requested payload and operation counts, not Private Bytes, Working Set, commit, or allocator-reserved memory. This workload proves candidate isolation and result equivalence only; cold start, stable idle, streaming, Agent load, context-pressure return-to-idle, Windows process-tree resources, and the same-host 30-run decision matrix remain required.
