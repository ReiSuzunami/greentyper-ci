...
The terminal matrix renders 27 canonical frames across 40x12, 80x24, and 160x50 viewports. Each size covers a baseline, model-equality no-op suppression, a status update, four streaming updates, the hierarchical Slash Panel, and a clearing frame that removes prior content. Both candidates resize a live renderer, replay their ANSI bytes through the same VT parser, and must reproduce every symbol, style, wide-character continuation, cleared cell, and final digest; every no-op produces zero bytes. The workload records render, resize, replay, byte, cell, and write-count evidence. It does not yet prove real ConPTY input, process startup, stable-idle CPU or memory, or the Performance Contract's P0/P1 budgets.

These are not storage or terminal decisions. Storage still requires cross-process CAS and migration interruption, SQLite VFS fault/corruption recovery, additional approval/tool-effect durability boundaries, fuzzing, same-commit 30-run FMDev evidence for every workload, and Windows directory-entry durability evidence. Terminal still requires Windows correctness beyond the in-memory VT oracle, same-commit FMDev runs, resource measurements, and a recorded selection. Transport and allocator candidates are not implemented.
...
Storage and terminal candidate versions are fixed in `Cargo.lock`. Planned transport and allocator versions were observed from the public crate index on 2026-08-09 and become fixed only when their candidate feature lands. Jemalloc is excluded from the primary Windows MSVC comparison because its maintained Rust wrappers do not present it as a supported MSVC target.
