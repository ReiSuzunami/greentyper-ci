...
The default runner compiles only `harness/sha256-loop`, which validates the pipeline. The optional `bench-storage` feature compiles SQLite WAL and checksummed append-log candidates against five explicit workloads: `critical-append-replay`, `bounded-streaming-replay`, `cas-one-winner`, `backup-restore`, and `interrupted-migration`. The runner requires `--workload`; evidence therefore cannot silently change boundaries while retaining only a comparison and implementation name.

The implemented matrix proves identical canonical replay, WAL `synchronous=FULL`, synchronous append-log commits, max-event bounded streaming batches, one-winner stale-head CAS policy, verified backup/restore, transactional or generation-based old-or-new migration recovery, exhaustive two-transaction tail-cut classification, checksum rejection, bounded framing, and per-run timing and size evidence. Migration interruption is currently an in-process transaction rollback or unpublished-generation simulation; it is not cross-process crash evidence.

This is not a storage decision. Timer-expiry streaming batches, cross-process crash injection, SQLite fault/corruption recovery, cross-process CAS contention, fuzzing, same-commit 30-run FMDev evidence for every workload, and Windows directory-entry durability evidence remain required. Transport, terminal, and allocator candidates are not implemented.
