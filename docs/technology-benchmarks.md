...
The default runner compiles only `harness/sha256-loop`, which validates the pipeline. The optional `bench-storage` feature compiles SQLite WAL and checksummed append-log candidates against the first `critical-append-replay` workload. It currently proves identical canonical replay, WAL `synchronous=FULL`, synchronous append-log commits, exhaustive two-transaction tail-cut classification, checksum rejection, bounded framing, and per-run timing and size evidence.

This is not a storage decision. Bounded streaming, cross-process crash injection, SQLite corruption recovery, CAS, backup/restore, interrupted migration, fuzzing, same-commit 30-run FMDev evidence, and Windows directory-entry durability evidence remain required. Transport, terminal, and allocator candidates are not implemented.
